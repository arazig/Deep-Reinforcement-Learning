...
class DQNAgent:
    ...
    def select_action(self, state: np.ndarray) -> np.ndarray:
        ...
        if not self.is_test:
            self.transition = [state_array, selected_action]
    ...
    # def test(self, get_frames=False, get_actions=False):
    #     """Test the agent."""
    #     self.is_test = True
    #     state = self.env.reset()
    #     done = False
    ...
    #     if get_frames and not get_actions:
    #         return score, frames
    #     elif get_actions and not get_frames:
    #         return score, actions
    #     elif get_frames and get_actions:
    #         return score, frames, actions
    #     else:
    #         return score

    # def test(self, video_folder: str = None, get_frames=False, get_actions=False):
    #     """Test the agent."""
    #     self.is_test = True
    ...
    def get_n_frames(self, frame: np.ndarray) -> np.ndarray:
        """Return the last n frames"""
        frame_array = frame[0] if isinstance(frame, tuple) else frame
        if self.frame_stack.full():
            self.frame_stack.stack(frame_array, 1)
            return self.frame_stack.frames
        else:
            self.frame_stack.stack(frame_array, self.n_frames_stack)
            return self.frame_stack.frames
